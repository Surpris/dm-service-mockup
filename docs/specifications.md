...
- [x] [Frontend] システム定義リレーションシップ追加機能の実装。
  - ユーザー定義リレーションシップ作成 UI に、システム定義リレーションシップを選択できるようなフォーム（プルダウンまたは検索）を追加する。
    - 接続されるノードの種類に応じて適切なリレーションシップがこのフォームで選択できる。
...
- [x] [Ops] `UserDefinedRelationship` から System Relation への昇格（Promotion）用SQL/スクリプトのプロトタイプ作成。
